...
**Goal:** 页面序列从「仅雷达页」扩展为「雷达页 + 独立视频页」混排，视频页支持色键（Chroma Key）抠像与底衬背景，预览与服务端渲染两端一致。

**Architecture:** `MultiPageSchema.pages` 从 `RadarVideoSchema[]` 演进为 `z.union([VideoPageSchema, RadarVideoSchema])[]`（视频页带判别字段 `pageType: "video"`，旧配置无该字段自然回落雷达页，零迁移）。新建 `VideoPage` 渲染组件（`@remotion/media` `<Video>` + `@remotion/effects` `colorKey`），接入 `MultiPageVideo` 的 Sequence 编排与 `Root` 的时长计算。媒体上传/预览 blob 替换/渲染端 URL 改写全部复用 #18 已落地的**按值匹配**管线，后端零改动。
...
| 1 | 数据层：`VideoPageSchema` + union + 类型守卫 + 时长函数 | 配置可存取，渲染未变 |
| 2 | 渲染层：`VideoPage` 组件 + `MultiPageVideo` 编排 + `Root` 时长 | 手改 JSON 可渲染含视频页的成片 |
| 3 | 编辑器：添加/配置/排序视频页 + 配对/数值/覆写面板兼容 + 预览 | 用户可视化完成全流程 |
...
// JSX：item.type === "video" → <VideoPage page={item.page} />
```
