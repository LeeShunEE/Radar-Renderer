...
<!-- generated at: 2026-06-04 | packages: backend (Python), frontend (TypeScript) -->
...
| 测试环境集成测试 | `tests/testenv-integration/backend/` | —— (前端无此阶段)                       | **用户手动**     | ✅ 真实数据库       |

**前端仅有两阶**：`unit` + `dev-integration`。**不存在** `tests/testenv-integration/frontend/`。前端的 testenv-integration 验证由 Claude Code 通过 MCP 工具**手动驱动**（启动服务 → 操作 Playwright → 读页面 → 断言），不写自动化 e2e 代码。
...
前端 testenv-integration 由 Claude Code 通过 MCP Playwright 工具手动执行：
  1. 启动 backend dev server
  2. 启动 frontend dev server
  3. 用 Claude Code 驱动浏览器走关键路径
```
...
| `tests/testenv-integration/backend/conftest.py` | testenv-integration 特化：`real_db_session` 等真库 fixture |
...
| service 领域层 | 自由命名（如 `Task`、`Order`） | 业务核心，**充血**（带行为/校验） |
